...
def generate_followup(conversation_text):
    """Generates exactly ONE follow-up question based on conversation so far."""
    import re

    prompt = f"""You are MediFind, a medical triage assistant.

Conversation so far: "{conversation_text}"

Rules (STRICT):
- Output EXACTLY one short question — nothing else
- No bullet points, no numbering, no intro phrase like "Sure!" or "To better understand..."
- Do NOT ask multiple questions in one message
- Do NOT use "Also" or "And" to sneak in a second question
- Do NOT repeat anything already answered
- Pick the FIRST aspect not yet covered: duration → severity (1-10) → exact location → triggers → other symptoms
...
Output: one question only."""
    ...
    response = client.chat.completions.create(
        model="llama-3.3-70b-versatile",
        messages=[
            {"role": "system", "content": "You output exactly one short follow-up question. No lists. No numbering. No extra text. One sentence only."},
            {"role": "user", "content": prompt}
        ],
        temperature=0.2
    )
    ...
    # Safety net: if model returns multiple lines, keep only the first non-empty one
